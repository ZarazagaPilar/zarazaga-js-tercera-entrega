    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (cliente) {
          Swal.fire(
            "Base de datos Kanus",
            `Se encuentra registrado como ${cliente.apellido.toUpperCase()} ${cliente.nombre.toUpperCase()}, propietario de ${
              cliente.nombreMascota
            } de ${cliente.edadMascota} años de edad.`
          );
        } else {
...
login.addEventListener("click", () => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      const dni = parseInt(document.getElementById("dni").value);
      const cliente = arrayClientes.find(
        (cliente) =>
          cliente.apellido.toLowerCase() === apellido && cliente.dni === dni
      );

      if (cliente) {
        Swal.fire(
          "¡Inicio de sesión exitoso!",
          `${cliente.nombre.toUpperCase()} eres una veterinaria autorizada.`,
          "success"
        ).then(() => {
